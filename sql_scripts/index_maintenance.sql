    ...
    CASE
        WHEN avg_fragmentation_in_percent > 80 AND TotalReads > 1000 THEN
            CASE WHEN IndexName IS NULL
                THEN 'ALTER INDEX ALL ON ' + QUOTENAME(SchemaName) + '.' + QUOTENAME(TableName) + ' REBUILD WITH (ONLINE = OFF, MAXDOP = 1);'
                ELSE 'ALTER INDEX ' + QUOTENAME(IndexName) + ' ON ' + QUOTENAME(SchemaName) + '.' + QUOTENAME(TableName) + ' REBUILD WITH (ONLINE = OFF, MAXDOP = 1);'
            END
        WHEN avg_fragmentation_in_percent BETWEEN 30 AND 80 AND TotalReads > 100 THEN
            CASE WHEN IndexName IS NULL
                THEN 'ALTER INDEX ALL ON ' + QUOTENAME(SchemaName) + '.' + QUOTENAME(TableName) + ' REORGANIZE;'
                ELSE 'ALTER INDEX ' + QUOTENAME(IndexName) + ' ON ' + QUOTENAME(SchemaName) + '.' + QUOTENAME(TableName) + ' REORGANIZE;'
            END
